...
train = bills_data[train_index, ]
ytrain = train$paid_in_full
Xtrain = train[, -train$paid_in_full]

test = bills_data[-train_index, ]
ytest = test$paid_in_full
Xtest = test[, -test$paid_in_full]

tree_mod = YARFCART(X = Xtrain, y = ytrain)

predict(tree_mod, Xtest)
```
...
```{r}
for (k in 2:10) {
  tree_mod = YARFCART(x, y, nodesize = k)
  rmse[k - 1] = sqrt(((predict(tree_mod, x) - ytest) ^ 2) / length(x))
}
