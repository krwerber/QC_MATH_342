...
```{r}
e1 = y - yhat
e2 = Hcomp %*% solve(t(Hcomp) %*% Hcomp) %*% t(Hcomp) %*% y
...
```{r}
random_matrix = matrix(nrow = nrow(X), ncol = ncol(X))
colnames(random_matrix) = colnames(X)

for (i in 1:nrow(random_matrix)) {
  random_matrix[i, 1:i] = lm(y ~ X[, 2:i])$coefficients
}
```
