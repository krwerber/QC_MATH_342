...
x_projection = Xjunk %*% solve(t(Xjunk) %*% Xjunk) %*% t(Xjunk)
I_n = diag(n)

expect_equal(c(x_projection), c(I_n))
```
...
sum_proj = 0
for (j in p_plus_one){
  sum_proj = sum_proj + orthogonal_projection(y, X[, j])$a_parallel
}
```
...
sqrt(sum(sum_proj ** 2)) / sqrt(sum(yhat ** 2))
```
...
Verify $Q^T Q$ is $Q_{p+1}$ i.e. $Q$ is orthonormal.
...
```{r}
proj_Q = Q %*% t(Q)
expect_equal(c(unname(proj_Q %*% y)), unname(yhat)) 
```
...
```{r}
x2 = X[, 2]
x2bin = cut(x2, breaks = quantile(x2, c(0, 0.5, 1)), include.lowest = TRUE)
table(x2bin)
```
